...
import { BookmarksQuickNav } from "@/components/BookmarksQuickNav";
...
function MinerProfileContent() {
  ...
  const handleToggleBookmark = () => {
    if (bookmarked) {
      ...
      success(`Removed from bookmarks`, {
        label: "Undo",
        onClick: () => addBookmark(pubkey),
      });
    } else {
      addBookmark(pubkey);
      success(`Added to bookmarks`, {
        label: "Undo",
        onClick: () => removeBookmark(pubkey),
      });
    }
  };

  // Keyboard shortcuts (B to bookmark, arrows to cycle)
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all ${
                bookmarked
                  ...
                  : "bg-slate-700 text-white hover:bg-amber-500 hover:text-black border border-slate-600 hover:border-amber-500"
              }`}
            >
              <svg
                className="w-5 h-5"
      ...
      {/* Search & Bookmarks Navigation */}
      <div className="mb-6 space-y-3">
        <MinerSearchBar 
          placeholder="Search miners..." 
          currentPubkey={pubkey}
        />
        <BookmarksQuickNav currentPubkey={pubkey} />
      </div>
