...
export function MinerSearchBar({
  ...
  const { bookmarks, addBookmark, removeBookmark } = useMinerBookmarks();
  ...
  const handleToggleBookmark = (e: React.MouseEvent, pubkey: string) => {
    e.stopPropagation();
    if (isBookmarked(pubkey)) {
              ...
              {filteredResults.map((miner, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {showBookmarkButtons && (
                      <button
                        onClick={(e) => handleToggleBookmark(e, miner.miner_pubkey)}
