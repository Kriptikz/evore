use evore::{
    ...
    state::{managed_miner_auth_pda, deployer_pda, autodeploy_balance_pda, Manager, Deployer, EvoreAccount},
};
...
// ============================================================================
// Deployer Account Helpers
// ============================================================================

/// Creates an Evore Deployer account with specified configuration
...
mod create_deployer {
    ...
    async fn test_success() {
        ...
        context.banks_client.process_transaction(tx).await.expect("create_deployer should succeed");
    ...
    #[tokio::test]
    async fn test_not_manager_authority() {
        let mut program_test = setup_programs();
        ...
        // Pre-create manager with manager_authority as authority
        add_manager_account(&mut program_test, manager_address, manager_authority.pubkey());
        ...
        // Try to create deployer with wrong signer
        let ix = evore::instruction::create_deployer(
            wrong_signer.pubkey(),
            manager_address,
            deploy_authority.pubkey(),
        ...
        let tx = Transaction::new_signed_with_payer(&[ix], Some(&wrong_signer.pubkey()), &[&wrong_signer], blockhash);
        let result = context.banks_client.process_transaction(tx).await;
        assert!(result.is_err(), "should fail when signer is not manager authority");
    }

    #[tokio::test]
    async fn test_deployer_already_exists() {
        let mut program_test = setup_programs();
        ...
        let tx = Transaction::new_signed_with_payer(&[ix], Some(&manager_authority.pubkey()), &[&manager_authority], blockhash);
        let result = context.banks_client.process_transaction(tx).await;
        assert!(result.is_err(), "should fail when deployer already exists");
    }
}
...
mod update_deployer {
    ...
    async fn test_success() {
        ...
        // Pre-create deployer with initial fee
        let (deployer_address, _) = deployer_pda(manager_address);
        add_deployer_account(
            &mut program_test,
            deployer_address,
        ...
        // Update deployer fee (keep same deploy_authority)
        ...
        let blockhash = context.banks_client.get_latest_blockhash().await.unwrap();
        let tx = Transaction::new_signed_with_payer(&[ix], Some(&manager_authority.pubkey()), &[&manager_authority], blockhash);
        context.banks_client.process_transaction(tx).await.expect("update_deployer should succeed");
    ...
    async fn test_not_manager_authority() {
        ...
        // Try to update deployer with wrong signer
        let ix = evore::instruction::update_deployer(
            wrong_signer.pubkey(),
            manager_address,
            deploy_authority.pubkey(),
...
mod mm_autodeploy {
    ...
    #[tokio::test]
    async fn test_success_with_10_percent_fee() {
        let mut program_test = setup_programs();
        
        // Different manager authority and deploy authority (the key scenario)
        let manager_authority = Keypair::new();
        let deploy_authority = Keypair::new();
        let manager_keypair = Keypair::new();
        let manager_address = manager_keypair.pubkey();
        ...
        // Pre-create manager (owned by manager_authority, NOT deploy_authority)
        add_manager_account(&mut program_test, manager_address, manager_authority.pubkey());
        
        // Pre-create deployer with deploy_authority
        add_deployer_account(
            &mut program_test,
            deployer_address,
            manager_address,
        ...
        let autodeploy_balance_funds = 1_000_000_000u64; // 1 SOL
        add_autodeploy_balance(&mut program_test, autodeploy_balance_address, autodeploy_balance_funds);
        
        // Setup deploy test accounts
        let current_slot = 1000;
        let _board = setup_deploy_test_accounts(&mut program_test, TEST_ROUND_ID, current_slot, 5);
        
        // Add ore miner for managed auth
        add_ore_miner_account(
            &mut program_test,
            managed_miner_auth.0,
            [0u64; 25],
        ...
        // Fund deploy_authority for tx fees only
        ...
        // Get balances before
        let deploy_authority_balance_before = context.banks_client.get_balance(deploy_authority.pubkey()).await.unwrap();
        let autodeploy_balance_before = context.banks_client.get_balance(autodeploy_balance_address).await.unwrap();
        let fee_collector_balance_before = context.banks_client.get_balance(FEE_COLLECTOR).await.unwrap();
        
        // Execute autodeploy with 10% expected fee
        ...
        context.banks_client.process_transaction(tx).await.expect("autodeploy should succeed");
        
        // Get balances after
        let deploy_authority_balance_after = context.banks_client.get_balance(deploy_authority.pubkey()).await.unwrap();
        let autodeploy_balance_after = context.banks_client.get_balance(autodeploy_balance_address).await.unwrap();
        let fee_collector_balance_after = context.banks_client.get_balance(FEE_COLLECTOR).await.unwrap();
        ...
        let expected_net_change = expected_deployer_fee as i64; // Fee received
        let actual_net_change = deploy_authority_balance_after as i64 - deploy_authority_balance_before as i64;
        // Account for tx fee (~5000 lamports)
        assert!(
            actual_net_change > expected_net_change - 100_000,
            "Deploy authority should receive deployer fee. Expected net ~{}, Got: {}",
            expected_net_change, actual_net_change
        );
        
        // Verify fee collector received protocol fee
        assert!(
            fee_collector_balance_after > fee_collector_balance_before,
            "Fee collector should receive protocol fee"
        );
        
        // Verify autodeploy_balance decreased (paid for deployment + fees)
        assert!(
            autodeploy_balance_after < autodeploy_balance_before,
            "Autodeploy balance should decrease. Before: {}, After: {}",
            autodeploy_balance_before, autodeploy_balance_after
        );
    }

    #[tokio::test]
    async fn test_wrong_deploy_authority() {
        let mut program_test = setup_programs();
        ...
        // Pre-create deployer with deploy_authority (not wrong_signer)
        add_deployer_account(
            &mut program_test,
            deployer_address,
            manager_address,
        ...
        add_autodeploy_balance(&mut program_test, autodeploy_balance_address, 1_000_000_000);
        ...
        // Fund wrong_signer
        let ix = system_instruction::transfer(&context.payer.pubkey(), &wrong_signer.pubkey(), 100_000_000);
        let blockhash = context.banks_client.get_latest_blockhash().await.unwrap();
        let tx = Transaction::new_signed_with_payer(&[ix], Some(&context.payer.pubkey()), &[&context.payer], blockhash);
        context.banks_client.process_transaction(tx).await.unwrap();
        
        // Try to autodeploy with wrong signer
        let cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(1_400_000);
        let ix = evore::instruction::mm_autodeploy(
            wrong_signer.pubkey(),
            manager_address,
        ...
        let tx = Transaction::new_signed_with_payer(&[cu_limit_ix, ix], Some(&wrong_signer.pubkey()), &[&wrong_signer], blockhash);
        let result = context.banks_client.process_transaction(tx).await;
        assert!(result.is_err(), "should fail when signer is not deploy_authority");
    }

    #[tokio::test]
    async fn test_unexpected_fee() {
        let mut program_test = setup_programs();
        ...
        // Pre-create deployer with 10% fee
        add_deployer_account(
            &mut program_test,
            deployer_address,
            manager_address,
        ...
        // Fund deploy_authority for tx fees
        let ix = system_instruction::transfer(&context.payer.pubkey(), &deploy_authority.pubkey(), 100_000_000);
        let blockhash = context.banks_client.get_latest_blockhash().await.unwrap();
        let tx = Transaction::new_signed_with_payer(&[ix], Some(&context.payer.pubkey()), &[&context.payer], blockhash);
        context.banks_client.process_transaction(tx).await.unwrap();
        ...
        let blockhash = context.banks_client.get_latest_blockhash().await.unwrap();
        let tx = Transaction::new_signed_with_payer(&[cu_limit_ix, ix], Some(&deploy_authority.pubkey()), &[&deploy_authority], blockhash);
        let result = context.banks_client.process_transaction(tx).await;
        assert!(result.is_err(), "should fail when expected_fee doesn't match deployer fee");
    }

    #[tokio::test]
    async fn test_zero_expected_fee_skips_check() {
        let mut program_test = setup_programs();
        ...
        let blockhash = context.banks_client.get_latest_blockhash().await.unwrap();
        let tx = Transaction::new_signed_with_payer(&[cu_limit_ix, ix], Some(&deploy_authority.pubkey()), &[&deploy_authority], blockhash);
        context.banks_client.process_transaction(tx).await.expect("autodeploy should succeed with expected_fee=0");
    }

    #[tokio::test]
    async fn test_deployer_not_initialized() {
        let mut program_test = setup_programs();
        ...
        // Fund deploy_authority
        let ix = system_instruction::transfer(&context.payer.pubkey(), &deploy_authority.pubkey(), 5_000_000_000);
        let blockhash = context.banks_client.get_latest_blockhash().await.unwrap();
        let tx = Transaction::new_signed_with_payer(&[ix], Some(&context.payer.pubkey()), &[&context.payer], blockhash);
        context.banks_client.process_transaction(tx).await.unwrap();
        
        // Try to autodeploy without deployer
        let cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(1_400_000);
        let ix = evore::instruction::mm_autodeploy(
            deploy_authority.pubkey(),
            manager_address,
        ...
        let tx = Transaction::new_signed_with_payer(&[cu_limit_ix, ix], Some(&deploy_authority.pubkey()), &[&deploy_authority], blockhash);
        let result = context.banks_client.process_transaction(tx).await;
        assert!(result.is_err(), "should fail when deployer is not initialized");
    }
    ...
    async fn test_round_ended() {
        ...
        // Try to autodeploy when round ended
        let cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(1_400_000);
        let ix = evore::instruction::mm_autodeploy(
            deploy_authority.pubkey(),
            manager_address,
    ...
    #[tokio::test]
    async fn test_no_squares_selected() {
        let mut program_test = setup_programs();
        ...
        // Try to autodeploy with no squares selected
        let cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(1_400_000);
        let ix = evore::instruction::mm_autodeploy(
            deploy_authority.pubkey(),
            manager_address,
        ...
        let tx = Transaction::new_signed_with_payer(&[cu_limit_ix, ix], Some(&deploy_authority.pubkey()), &[&deploy_authority], blockhash);
        let result = context.banks_client.process_transaction(tx).await;
        assert!(result.is_err(), "should fail when no squares are selected");
    }

    #[tokio::test]
    async fn test_insufficient_autodeploy_balance() {
        let mut program_test = setup_programs();
        ...
        add_autodeploy_balance(&mut program_test, autodeploy_balance_address, 1_000); // Only 1000 lamports
        ...
        // Try to autodeploy with insufficient balance
        let cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(1_400_000);
        let ix = evore::instruction::mm_autodeploy(
            deploy_authority.pubkey(),
            manager_address,
        ...
        let tx = Transaction::new_signed_with_payer(&[cu_limit_ix, ix], Some(&deploy_authority.pubkey()), &[&deploy_authority], blockhash);
        let result = context.banks_client.process_transaction(tx).await;
        assert!(result.is_err(), "should fail when autodeploy_balance has insufficient funds");
    }
}
